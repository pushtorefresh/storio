...
/**
 * Annotation processor for StorIOSQLite. <p> It'll process annotations to generate StorIOSQLite
 * Object-Mapping. <p> Addition: Annotation Processor should work fast and be optimized because it's
...
public class StorIOSQLiteProcessor extends StorIOAnnotationsProcessor<StorIOSQLiteTypeMeta,
  ...
  @NotNull
  @Override
  protected StorIOSQLiteTypeMeta processAnnotatedClass(@NotNull TypeElement classElement,
      @NotNull Elements elementUtils) {
    final StorIOSQLiteType storIOSQLiteType
        = classElement.getAnnotation(StorIOSQLiteType.class);
  ...
  protected void processAnnotatedFieldsOrMethods(@NotNull final RoundEnvironment roundEnvironment,
      @NotNull Map<TypeElement, ? extends StorIOSQLiteTypeMeta> annotatedClasses) {
    ...
    for (final Element annotatedFieldElement : elementsAnnotatedWithStorIOSQLiteColumn) {
      try {
        ...
        final StorIOSQLiteColumnMeta storIOSQLiteColumnMeta
            = processAnnotatedFieldOrMethod(annotatedFieldElement);

        final StorIOSQLiteTypeMeta storIOSQLiteTypeMeta
            = annotatedClasses.get(storIOSQLiteColumnMeta.getEnclosingElement());

        // If class already contains column with same name -> throw an exception.
        if (storIOSQLiteTypeMeta.getColumns().containsKey(
            storIOSQLiteColumnMeta.getStorIOColumn().name())) {
        ...
        if ((storIOSQLiteTypeMeta.getNeedCreator() && !storIOSQLiteColumnMeta.isMethod())
            || (!storIOSQLiteTypeMeta.getNeedCreator()
            && storIOSQLiteColumnMeta.isMethod()
            && !storIOSQLiteTypeMeta.getColumns().isEmpty())) {
          throw new ProcessingException(annotatedFieldElement,
              "Can't apply "
                  + StorIOSQLiteColumn.class.getSimpleName()
                  + " annotation to both fields and methods in a same class: "
                  + storIOSQLiteTypeMeta.getSimpleName()
          );
        }

        // If column needs creator then enclosing class needs it as well.
        if (!storIOSQLiteTypeMeta.getNeedCreator() && storIOSQLiteColumnMeta.isMethod()) {
          storIOSQLiteTypeMeta.setNeedCreator(true);
        }

        // Put meta column info.
        storIOSQLiteTypeMeta.getColumns().put(storIOSQLiteColumnMeta.getStorIOColumn().name(),
            storIOSQLiteColumnMeta);
  ...
  @NotNull
  @Override
  protected StorIOSQLiteColumnMeta processAnnotatedFieldOrMethod(
      @NotNull final Element annotatedField) {
    final JavaType javaType;

    try {
      javaType = JavaType.Companion.from(annotatedField.getKind() == ElementKind.FIELD
      ...
      throw new ProcessingException(annotatedField,
          "Unsupported type of field or method for "
              + StorIOSQLiteColumn.class.getSimpleName()
              + " annotation, if you need to serialize/deserialize"
              + " field of that type "
    ...
    final StorIOSQLiteColumn storIOSQLiteColumn = annotatedField.getAnnotation(
        StorIOSQLiteColumn.class);
  ...
  protected void processAnnotatedExecutables(@NotNull RoundEnvironment roundEnvironment,
    ...
    for (final Element annotatedElement : elementsAnnotatedWithStorIOSQLiteCreator) {
      final ExecutableElement annotatedExecutableElement
      ...
      final StorIOSQLiteTypeMeta storIOSQLiteTypeMeta =
          annotatedClasses.get(storIOSQLiteCreatorMeta.getEnclosingElement());

      // Put meta creator info.
      // If class already contains another creator -> throw exception.
      if (storIOSQLiteTypeMeta.getCreator() == null) {
        storIOSQLiteTypeMeta.setCreator(annotatedExecutableElement);
      } else {
  ...
  @Override
  protected void validateAnnotatedClassesAndColumns(@NotNull Map<TypeElement,
      ? extends StorIOSQLiteTypeMeta> annotatedClasses) {
    // Check that each annotated class has columns with at least one key column.
    for (final Map.Entry<TypeElement, ? extends StorIOSQLiteTypeMeta> annotatedType
        : annotatedClasses.entrySet()) {
      final StorIOSQLiteTypeMeta storIOSQLiteTypeMeta = annotatedType.getValue();

      if (storIOSQLiteTypeMeta.getColumns().size() == 0) {
      ...
      if (storIOSQLiteTypeMeta.getNeedCreator() && storIOSQLiteTypeMeta.getCreator() == null) {
        throw new ProcessingException(annotatedType.getKey(),
            "Class marked with "
                + StorIOSQLiteType.class.getSimpleName()
      ...
      if (storIOSQLiteTypeMeta.getNeedCreator()
          && storIOSQLiteTypeMeta.getCreator().getParameters().size()
          != storIOSQLiteTypeMeta.getColumns().size()) {
        throw new ProcessingException(annotatedType.getKey(),
            "Class marked with "
                + StorIOSQLiteType.class.getSimpleName()
                + " annotation needs factory method or constructor marked with "
