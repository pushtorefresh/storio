...
object GetResolverGenerator : Generator<StorIOSQLiteTypeMeta> {
    ...
    override fun generateJavaFile(typeMeta: StorIOSQLiteTypeMeta): JavaFile {
        ...
        val getResolver = TypeSpec.classBuilder(generateName(typeMeta))
                ...
                .superclass(ParameterizedTypeName.get(ClassName.get(
                        "com.pushtorefresh.storio.sqlite.operations.get", "DefaultGetResolver"),
                        className))
                .addMethod(
    ...
    private fun createMapFromCursorMethodSpec(typeMeta: StorIOSQLiteTypeMeta,
        ...
        val builder = MethodSpec.methodBuilder("mapFromCursor")
                ...
                .addModifiers(PUBLIC)
                .returns(className)
                .addParameter(ParameterSpec.builder(ClassName.get("android.database", "Cursor"),
                        "cursor")
                        .addAnnotation(ANDROID_NON_NULL_ANNOTATION_CLASS_NAME)
                        .build())
    ...
    private fun createMapFromCursorWithCreatorMethodSpec(typeMeta: StorIOSQLiteTypeMeta,
                                                         className: ClassName): MethodSpec {
        val builder = MethodSpec.methodBuilder("mapFromCursor")
                .addJavadoc("{@inheritDoc}\n")
                .addAnnotation(Override::class.java)
        ...
        var first = true
        for (columnMeta in typeMeta.orderedColumns) {
            val columnIndex = "cursor.getColumnIndex(\"${columnMeta.storIOColumn.name}\")"
        ...
        if (typeMeta.creator!!.kind == ElementKind.CONSTRUCTOR) {
            ...
            builder.addStatement("\$T object = \$T.\$L", className, className,
                    "${typeMeta.creator!!.simpleName}$paramsBuilder")
        }
    ...
    private val SUFFIX = "StorIOSQLiteGetResolver"
